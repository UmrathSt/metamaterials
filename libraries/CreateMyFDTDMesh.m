...
PMLthickness = 8;
if sGeom.PML == 3;
    PMLthickness = sGeom.PMLthickness;
end;
PMLdirections = [0, 0, 0, 0, 0, PMLthickness];
if strcmp(sGeom.grounded, 'False');
    zmin = -sGeom.z_size;
    PMLdirections = [0, 0, 0, 0, PMLthickness, PMLthickness];
    while abs(zmin/sGeom.maxres.z) < 10+PMLthickness;
        zmin = zmin-sGeom.maxres.z;
    end;
end;
zmax = lz_structure+sGeom.z_size;
while (zmax-lz_structure)/sGeom.maxres.z < 10+PMLthickness;
