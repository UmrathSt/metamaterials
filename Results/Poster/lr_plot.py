...
gtop = c.insert(graph.graphxy(width=width, height=width/gratio,xpos=gbottom.width+0.5,
		x = graph.axis.lin(painter=xgridpainter, title="$f$ [GHz]",
                    min=1,max=fmax),
                y2 = graph.axis.lin(title="Reflexionskoeffizient $|R|^2$",max=1,min=0,
                    painter=ygridpainter)
                )
             )

gtop.plot([graph.data.values(x = fA, y = dataA[:,1]**2)],
